...
function wrapEmitWithTransaction(agent, emit, isHTTPS) {
  ...
  return tracer.transactionProxy(function wrappedHandler(evnt, request, response) {
    ...
    function instrumentedFinish() {
      ...
      console.log('clearing txInfo')
      if (request[TRANSACTION_INFO_KEY]) {
        request[TRANSACTION_INFO_KEY] = null
      }


      // request.__NR_segment = null

      if (response[TRANSACTION_INFO_KEY]) {
        response[TRANSACTION_INFO_KEY] = null
      }

      // response.__NR_segment = null
    }
