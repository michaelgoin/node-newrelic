...
Object.defineProperty(Tracer.prototype, 'segment', {
  ...
  set: function segmentSetter(segment) {
    const isNull = !!segment
    logger.error('isNull: ', isNull)
...
Tracer.prototype.printMap = function printMap() {
  logger.error(idMap.size)
}
...
function _makeWrapped(tracer, handler, active, full) {
  ...
  function wrapped() {
    ...
    let newActiveHandle = active

    var prev = tracer.segment
    tracer.segment = newActiveHandle
    if (newActiveHandle && full) newActiveHandle.start()
    try {
      return handler.apply(this, arguments)
    } catch (err) {
      ...
      if (newActiveHandle && full) newActiveHandle.touch()
      tracer.segment = prev

      // TODO: why the fuck does setting this to null result in crashes?
      // could this function get invoked twice? But we've cleared the capture?
      newActiveHandle = null
    }
  }
}
